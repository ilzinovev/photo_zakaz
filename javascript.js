	...
	function addImage(ind) {
		...
		for (i = start; i < end; i++) {
			...
			if($('#dropped-files > .image').length <= maxFiles) {
				$('#dropped-files').append('<div id="img-'+i+'" class="image" style="display:inline-block;background: url('+dataArray[i].value+'); background-size: cover;"> <a href="'+dataArray[i].value+'" data-fancybox><img id="'+i+'" src="'+dataArray[i].value+'"style="opacity:0.0;filter:alpha(opacity=0)" width=100%></a><div class="drop-button">копии&nbsp;&nbsp;<input type="text" value="1" size="1" placeholder="кол вол копий данной фото"/><datalist id="333"><option value="1">1</option></datalist><a href="#" id="drop-'+i+'" >&nbsp;&nbsp;&nbsp;&nbsp;Удалить изображение</a></div><br></div>');
//$('#dropped-files').append('<br><input type="text" value="1" size="1" placeholder="кол вол копий данной фото"/>');
			}
		}
		return false;
	...
	$("#dropped-files").on("click","div[id^='img']", function() {
		// получаем название id
		var elid = $(this).attr('id');
		// создаем массив для разделенных строк
	...
	// // $('#preview').append('<div id="img-'+elid+'" class="image" style="background: url('+dataArray[elid].value+');width:600px;height:1200px; background-size: contain;"><a href=>ляля</a> <a href="#" id="drop-'+i+'" class="drop-button"></a></div>');
  //  $('#preview').append('<a href="'+dataArray[elid].value+'" data-fancybox><img id="'+elid+'" src="'+dataArray[elid].value+'"></a>');
	//
